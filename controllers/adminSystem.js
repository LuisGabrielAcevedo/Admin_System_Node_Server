...
const apiProductTypesData = require("../data/apiProductTypes");
const apiRoomTypesData = require("../data/apiRoomTypes");
const currenciesDate = require("../data/currencies");
...
function randomProducts(req, res) {
  ...
  const foodFast = {
    products: ["pizza napolitana", "hamburguesa", "coca cola"],
    application: "5cca22d05084d906c1ffb022",
    company: "5ccd969611e62a5eb000b0dd"
  };
  Promise.all([saveProducts(musicalInstruments), saveProducts(foodFast)])
    .then(resp => {
      res.status(200).send({
        status: "OK",
        code: 200,
...
function mercadoLibreApi(value) {
  return new Promise((resolve, reject) => {
    Axios.get(
      `https://api.mercadolibre.com/sites/MLA/search?q=:${value}&limit=2`
    ).then(resp => {
...
// 10. Room types
function roomTypes(req, res) {
  res.status(200).send({
    status: "OK",
    code: 200,
    msg: "Lista de tipos de cuartos (API) encontrada",
    data: apiRoomTypesData
  });
}
