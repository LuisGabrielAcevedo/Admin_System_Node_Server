function validationsMiddlewareFunction(req, res, next) {
  ...
  if (Object.entries(body).length === 0 && body.constructor === Object) {
    return res.status(422).send({
      status: "WARNING",
      code: 422,
      msg: `The body is empty, please send data to ${
    ...
    if (errorsStatus) {
      ...
      if (route === "users" && method === "post") {
        ...
        if (
          !["5ce01e90faf3940017b6b0b2", "5cca3732a342520bbcd24563"].includes(
            String(req.tokenVerified.application._id)
          )
        ) {
          return res.status(422).send({
            status: "WARNING",
...
const rooms = require("../models/inventory/room");
