  ...
  return new Promise((resolve, reject) => {
    ...
    switch (type) {
      case "IMAGE_CUSTOMER":
        ...
        url = `${config.server.urlApi}customers/${id}/image/${fileName}`;
        break;
      case "IMAGE_USER":
        pathToSave = `uploads/user`;
        url = `${config.server.urlApi}users/${id}/image/${fileName}`;
        break;
      case "IMAGE_COMPANY":
        pathToSave = `uploads/company/image`;
        url = `${config.server.urlApi}companies/${id}/image/${fileName}`;
        break;
      case "LOGO_COMPANY":
      ...
      case "IMAGE_PRODUCT":
        ...
        url = `${config.server.urlApi}products/${id}/image/${fileName}`;
        break;
    }
    ...
    fs.stat(directory, error => {
      if (error) {
        if (error.code === "ENOENT") {
          ...
          reject({
            msg:
              "Error verificancdo el tipo de error de existencia del directorio"
          });
        }
      } else {
        resolve({ directory: directory });
...
function getFile(payload) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      !payload.hasOwnProperty("path") ||
      !payload.hasOwnProperty("fileName")
    )
      return reject({
        status: "ERROR",
        code: 500,
