...
function save(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `save_${payload.collection.modelName.toLowerCase()}_successs`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `save_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
...
function findCollection(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `find_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `find_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
...
function findByIdCollection(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `findbyid_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `findbyid_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
    // 1. Collection validate
    if (!payload.collection)
      return reject({
...
function pushIdCollection(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `update_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `update_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
...
function pullIdCollection(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `delete_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `delete_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
    // 1. Validar que venga el campo id
    if (!payload.hasOwnProperty("id"))
      return reject({
...
function updateManyIds(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `updated_ids_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `updated_ids_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
...
function removeCollection(payload) {
  const msgSuccess = payload.successMessage
    ? payload.successMessage
    : `delete_collection_${payload.collection.modelName.toLowerCase()}_success`;
  const msgError = payload.errorMessage
    ? payload.errorMessage
    : `delete_collection_${payload.collection.modelName.toLowerCase()}_error`;
  return new Promise((resolve, reject) => {
    // 1. Validar coleccion
    if (!payload.collection)
      return reject({
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!payload.files)
      return reject({
        status: "WARNING",
        code: 422,
        msg: `it_is_necessary_to_attach_a_file`
      });
    ...
    if (!payload.hasOwnProperty("fieldToEdit"))
      return reject({
        status: "WARNING",
        code: 422,
        msg: `The_field_fieldToEdit_is_required_to_complete_the_update_process. Example: fieldToEdit: 'profileImage'. Se refiere al campo donde va la imagen en el objeto`
      });
    payload.collection.findById(payload.id).exec((err, dataBaseResp) => {
      if (err)
        return reject({
      ...
      fileMethods
        .saveImage(payload)
        .then(saveImageResp => {
          dataBaseResp[payload.fieldToEdit] = saveImageResp._id;
          payload.collection.findByIdAndUpdate(
            dataBaseResp._id,
            dataBaseResp,
            (err, dataBaseResp1) => {
              dataBaseResp1[payload.fieldToEdit] = saveImageResp;
              if (err)
                return reject({
                  status: "ERROR",
                  code: 500,
